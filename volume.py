...
while True:
    ...
    detect.findHands(img, False)
    lmList, bbox = detect.findPosition(img, draw=False)
    fingers = detect.fingerDown()

    if len(lmList) !=0:
        if (20 not in fingers):
            count = 0
        if fingers == [20]:
            if count == 0:
                count = time.time()
            
            if int(time.time()-count) == 3:
                volCheck = not volCheck
                count =0
        ...
        if fingers == [12,16,20] and volCheck:
